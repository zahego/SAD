function setOtherDeleteContent(httpRequest) {
                        ...
                        for (var i = 0; i < obj.webUserList.length; i++) {
                            ...
                            obj.webUserList[i].delete = "<img src='icons/delete.png'  onclick='callOtherDeleteAPI(" + id + ",this)'  />";
                    ...
                    function callOtherDeleteAPI(id, icon) {
                        if (confirm("Do you really want to delete game class number " + id + "? ")) {
                            console.log("icon that was passed into JS function is printed on next line");
                            console.log(icon);
                     ajaxCall("webAPIs/deleteOtherAPI.jsp?deleteOtherId=" + id, OtherAPISuccess, OtherAPIError);

                    function OtherAPISuccess(httpReq) { // function is local to callDeleteAPI
                        var obj = JSON.parse(httpReq.responseText);
                        alert("Web API success. Message is [" + obj.errorMsg + "] -- empty string means success.");
                    }

                    function OtherAPIError(httpReq) { // function is local to callDeleteAPI
